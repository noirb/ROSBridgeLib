...
public class ROSBridgeMsg
{
    ...
    public static string CallService(string serviceName, string id, string serviceArgs)
    {
        if (string.IsNullOrEmpty(serviceArgs)) serviceArgs = "{}";
        return @"{""op"" : ""call_service"", ""id"": """ + id + @""", ""service"": """ + serviceName + @""", ""args"": " + serviceArgs + @"}";
    }
